...
│   ├── CMakeLists.txt
│   └── ui/
│       ├── ui_theme.h/c    # 8 套颜色主题 + 对比度/色块/图标工具
│       ├── ui_lang.h       # 中英文案（CFG_LANG 编译期切换）
│       ├── ui_monitor.h/c  # 监控页面（8 种风格布局 + 分页逻辑）
│       ├── style_*.c       # 各风格的布局实现
...
共 8 套 UI 风格（设计描述见 [UI-DESIGN.md](UI-DESIGN.md)），编译时由 `config.h` 的 `CFG_UI_STYLE` 宏选择 1 种链接进固件：
...
| `STYLE_SSD` | style_ssd.c | 固态硬盘标签风（2.5 寸 SSD 盘面） |
