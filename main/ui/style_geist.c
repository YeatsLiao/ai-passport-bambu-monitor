// main/ui/style_geist.c —— 风格11: Geist 控制台风（纯黑 + Vercel 蓝 + 发丝线分区）
//
// 设计语言 (Vercel Geist / emil-design-eng 哲学):
//   - 无卡片描边堆叠: 平面分区 + 1px 发丝线分隔, 让层级靠排版而非边框
//   - 排版层级: 进度大数字 (48pt) 是唯一视觉主角, 其余为左灰标签 / 右对齐白值两栏
//   - 对齐是隐形细节: 同一列数值全部右对齐, 不逐个估算
//   - 克制动画: 翻页 150ms 淡入 + 8px 上移 ease-out (偶发操作才动画, 高频按键无动画)
...
static const char *TAG __attribute__((unused)) = "style_geist";
...
#define HAIRLINE     2     // 发丝线粗细 (1px 在 240px 屏上过细, 2px 仍显细腻)
...
// Page 0 组件
static lv_obj_t *s_pct_lbl = NULL;    // 大数字进度 (唯一视觉主角)
static lv_obj_t *s_bar = NULL;        // 细进度条
...
static lv_obj_t *s_ams_bar[5];        // 行底细余量条
...
// 入场动画: 淡入 + 8px 上移, 150ms ease-out
// (emil: 入场用 ease-out 起步快, <300ms, 从 opacity 0 + 微位移出现而非凭空闪现)
static void anim_enter(lv_obj_t *card) {
    lv_anim_t a;
    lv_anim_init(&a);
    lv_anim_set_var(&a, card);
    lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t)lv_obj_set_y);
    lv_anim_set_values(&a, CARD_Y + 8, CARD_Y);
...
// 2px 细进度条 (Geist 发丝线质感)
static lv_obj_t *mk_thin_bar(lv_obj_t *p, lv_coord_t x, lv_coord_t y, lv_coord_t w) {
    if (!p) return NULL;
    lv_obj_t *bar = lv_bar_create(p);
    if (!bar) return NULL;
    lv_obj_set_pos(bar, x, y);
    lv_obj_set_size(bar, w, HAIRLINE);
    const ui_theme_colors_t *c = ui_theme_get_colors();
    lv_obj_set_style_bg_color(bar, lv_color_hex(c->border), 0);   // 轨道 = 发丝线灰
...
// 布局: 小灰标签 "PROGRESS" → 48pt 大数字 → 细进度条 → 数据行 (左标签/右值)
// ---------------------------------------------------------------------------
static void build_page0(void) {
    ...
    lv_obj_set_style_bg_opa(card, LV_OPA_TRANSP, 0);   // 平面分区: 无卡片底色
    lv_obj_set_style_pad_all(card, 0, 0);
    lv_obj_set_style_border_width(card, 0, 0);
    lv_obj_remove_flag(card, LV_OBJ_FLAG_SCROLLABLE);

    // 进度区: 小标签 + 大数字 + 细条
    mk_lbl(card, L_PROGRESS, L_FONT_TEXT, c->text_secondary);

    s_pct_lbl = mk_lbl(card, "--%", L_FONT_NUM_HUGE, c->text_primary);
    if (s_pct_lbl) lv_obj_set_pos(s_pct_lbl, 0, 14);

    s_bar = mk_thin_bar(card, 0, 70, CARD_W);

    // 数据行: CFG_COMPONENT_ORDER 中除 PERCENT 外的组件, 每行 30px
    // (PERCENT 已占进度区主角位, 无论在 order 中的位置如何都不再重复成行)
    int y = 84;
    int row = 0;
    ...
    for (int i = 0; i < s_order_len && row < 6; i++) {
        ...
        // 左标签灰 / 右数值白: 两栏对齐, 行与行共用同一组 y, 产生对齐复利
        s_row_lbl[row] = mk_lbl(card, buf, L_FONT_TEXT, c->text_secondary);
        if (s_row_lbl[row]) {
        ...
        if (i + 1 < s_order_len && s_order[i + 1] != CMP_PERCENT) {
            lv_obj_t *rule = lv_obj_create(card);
            if (rule) {
                lv_obj_set_pos(rule, 0, y + 29);
                lv_obj_set_size(rule, CARD_W, 1);
                lv_obj_set_style_bg_color(rule, lv_color_hex(c->border), 0);
                lv_obj_set_style_bg_opa(rule, LV_OPA_50, 0);
                lv_obj_set_style_radius(rule, 0, 0);
                lv_obj_set_style_border_width(rule, 0, 0);
                lv_obj_set_style_pad_all(rule, 0, 0);
                lv_obj_remove_flag(rule, LV_OBJ_FLAG_SCROLLABLE | LV_OBJ_FLAG_CLICKABLE);
            }
        }
        y += 30;
        row++;
...
static void build_page1(void) {
    ...
    mk_lbl(card, L_AMS, L_FONT_TEXT_BIG, c->text_primary);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        s_ams_val[i] = mk_lbl(card, "--", L_FONT_NUM, c->text_primary);
        mk_row_right(s_ams_val[i], y);

        s_ams_bar[i] = mk_thin_bar(card, 0, y + 24, CARD_W);
    }
}
...
void style_geist_build(void) {
    ...
    // 发丝线只画底边: header 与内容区之间的层级靠这条线, 不靠色块差
    ...
    // 页码用强调色: Vercel 蓝只出现在"活动位置"上 (页码/进度条), 保持克制
...
void style_geist_update(void) {
        ...
        for (int i = 0; i < s_order_len && row < 6; i++) {
            ...
            switch (cmp) {
                ...
                case CMP_STATE: {
                    ...
                    if (conn) {
                        snprintf(buf, sizeof(buf), "%s", state_text(st->state));
                    } else {
                        snprintf(buf, sizeof(buf), "%s", L_CONNECTING);
                    }
                    lv_label_set_text(val, buf);
                    // 状态色由实时数据驱动 (左标签的 WIFI 图标随右值同步变色)
    ...
    if (s_page == 1) {
        for (int i = 0; i < 5; i++) {
            ...
            char name[8];
            if (i < 4) snprintf(name, sizeof(name), "A%d", i + 1);
...
// ---------------------------------------------------------------------------
int style_geist_page_count(void) { return s_total_pages; }
int style_geist_current_page(void) { return s_page; }
