// main/ui/style_industrial.c —— 风格5: 硬核工控风（深灰黑 + 绿/黄/红状态灯，无圆角）
...
static void build_page0(void) {
    ...
    lv_obj_set_pos(card, 4, 34);
    lv_obj_set_size(card, 232, 216);
    ...
    for (int i = 0; i < s_order_len && i < 6; i++) {
        ...
        switch (cmp) {
            ...
            case CMP_STATE:    color = c->success; snprintf(buf, sizeof(buf), "[OK] " L_STATE_IDLE); break;
...
static void build_page1(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_set_pos(s_ams_lbl[i], 0, 32 + i * 24);
    }
}
...
void style_industrial_build(void) {
    ...
    mk_lbl(header, L_TITLE_BAMBU, &lv_font_montserrat_14, c->text_primary);
    ...
    for (int i = 0; i < s_order_len && i < 6; i++) {
        ...
        switch (cmp) {
            ...
            case CMP_STATE: {
                ...
                uint32_t sc = c->success;
                if (st->state == BAMBU_STATE_PAUSE) sc = c->warning;
                else if (st->state == BAMBU_STATE_FAILED) sc = c->error;
                else if (st->state != BAMBU_STATE_RUNNING) sc = c->text_secondary;
                snprintf(buf, sizeof(buf), "[%s] %s",
                    st->state == BAMBU_STATE_RUNNING ? "OK" :
                    st->state == BAMBU_STATE_PAUSE ? "WARN" :
                    st->state == BAMBU_STATE_FAILED ? "ERR" : "--", txt);
                lv_label_set_text(s_lbl[i], buf);
                lv_obj_set_style_text_color(s_lbl[i], lv_color_hex(sc), 0);
                break;
            }
            case CMP_SPEED:
...
void style_industrial_next_page(void) { s_page = (s_page + 1) % s_total_pages; rebuild_page(); }
void style_industrial_prev_page(void) { s_page = (s_page - 1 + s_total_pages) % s_total_pages; rebuild_page(); }
