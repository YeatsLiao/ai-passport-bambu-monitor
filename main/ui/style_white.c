...
static void build_page1(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_set_pos(s_ams_lbl[i], 0, 36 + i * 28);
    }
}
...
void style_white_next_page(void) { s_page = (s_page + 1) % s_total_pages; rebuild_page(); }
void style_white_prev_page(void) { s_page = (s_page - 1 + s_total_pages) % s_total_pages; rebuild_page(); }
